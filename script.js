...
function seedLevel(n) {
  ...
  for (let k = 0; k < Math.min(8, n * 2); k++) { let p = randomEmpty(); obstacles.push(p); }
...
function canMove(x, y) { 
  if (x < 0 || x >= COLS || y < 0 || y >= ROWS) 
    return false; 
  if (obstacles.some(o => o.x === x && o.y === y)) return false; return true; 
  console.log('Movimento permitido.');
}

function moveEntity(ent, dir) { 
  let nx = ent.x, ny = ent.y; 
  if (dir === 0) ny--; if (dir === 1) nx++; 
  if (dir === 2) ny++; if (dir === 3) nx--; if (canMove(nx, ny)) { ent.x = nx; ent.y = ny; } 
...
function enemyStep() {
  ...
  const caught = (enemy.x === player.x && enemy.y === player.y);

  // Check if enemy collected any dishes
  let collectedDish = false;
  for (let i = dishes.length - 1; i >= 0; i--) {
    const d = dishes[i];
    if (d.x === enemy.x && d.y === enemy.y) {
      dishes.splice(i, 1);
      collectedDish = true;
...
window.addEventListener('keydown', (e) => {
  ...
  if (k === 'ArrowUp' || k === 'w' || k === 'W') { if (canMove(player.x, player.y - 1)) { player.y--; moved = true; } }
  if (k === 'ArrowDown' || k === 's' || k === 'S') { if (canMove(player.x, player.y + 1)) { player.y++; moved = true; } }
  if (k === 'ArrowLeft' || k === 'a' || k === 'A') { if (canMove(player.x - 1, player.y)) { player.x--; moved = true; } }
  if (k === 'ArrowRight' || k === 'd' || k === 'D') { if (canMove(player.x + 1, player.y)) { player.x++; moved = true; } }
  if (moved) {
    for (let i = dishes.length - 1; i >= 0; i--) {
...
function preTrain(episodes = 1000) {
  ...
  for (let e = 0; e < episodes; e++) {
    ...
    for (let step = 0; step < 60; step++) {
      ...
      const caught = (simEnemy.x === simPlayer.x && simEnemy.y === simPlayer.y);
      
      // Check if enemy collected dishes
      let collectedDish = false;
